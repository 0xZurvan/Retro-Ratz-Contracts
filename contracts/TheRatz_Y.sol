...
contract TheRatz_Y is ERC721Enumerable, Ownable{
    ...
    function checkTokenLevel(uint256 _tokenId) external view returns(uint256) {
    ...
    function setAllRiddleAnswers(bytes32[] memory _answer) external onlyOwner {
        uint256 _index = 0;
    ...
    function updateRiddleAnswer(uint256 _level, bytes32 _newAnswer) external onlyOwner {
        levelToRiddleHash[_level] = _newAnswer;
    }
    ...
    // Level up you token by cracking the code and paying with Cheeze. 
    ...
    function levelUpWithPayment(string memory _response, uint256 _tokenId) external payable returns(uint256 newLevel) {
    ...
    function updateBaseExtension(string memory _baseExtension) external onlyOwner {
    ...
    // The actual Uniform Resource Identifier (URI) for tokenId token.
